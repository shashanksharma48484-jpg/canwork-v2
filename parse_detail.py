...
def parse_statcan_page(html_path: str) -> str:
    """Parse a StatCan NOC detail page into clean Markdown."""
    with open(html_path, encoding="utf-8", errors="ignore") as f:
        soup = BeautifulSoup(f.read(), "html.parser")

    md = []

    # Title — use the h2 with NOC code like "00010 - Legislators"
    title_h2 = soup.find("h2", class_="bg-def-1")
    if title_h2:
        # Strip the code prefix: "00010 - Legislators" -> "Legislators"
        raw = clean(title_h2.get_text())
    ...
    # Content is in <li class="list-unstyled"> blocks
    # Each block starts with a <strong> label, followed by content
    target_labels = [
    ...
    for li in soup.find_all("li", class_="list-unstyled"):
        strong = li.find("strong")
        if not strong:
            continue
        label = clean(strong.get_text())
        if not any(t.lower() in label.lower() for t in target_labels):
            continue

        md.append(f"\n## {label}\n")

        # Extract all text/list content inside this li
        for elem in li.children:
            if not hasattr(elem, 'name'):
                continue
            if elem.name == "p":
                # skip the label paragraph itself
                if elem.find("strong"):
                    continue
                text = clean(elem.get_text())
                if text:
                ...
                for item in elem.find_all("li"):
                    md.append(f"- {clean(item.get_text())}")
                md.append("")
